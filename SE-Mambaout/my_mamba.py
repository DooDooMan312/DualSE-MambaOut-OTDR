...
from model_utils.train_v2_nomatrix import train_VAL
from torch.amp import autocast, GradScaler
from loss_separation import SeparationLoss, SeparationLossConfig
import os
...
# ------------------------- loss (key change) -------------------------
loss_cfg = SeparationLossConfig(
    class_a=1,
    class_b=2,
    ohem_ratio=1.0,      # 先关掉 OHEM（非常关键）
    margin=0.5,          # 从 1.0 降到 0.5
    margin_lambda=0.05,  # 从 0.2 降到 0.05（非常关键）
    label_smoothing=0.0,
    class_weights=None
)
loss_fn = SeparationLoss(cfg=loss_cfg)
