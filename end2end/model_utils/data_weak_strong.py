...
# 数据增强配置（适配 numpy 输入：HxWxC 的 uint8）
weak_transform = transforms.Compose([
    transforms.ToPILImage(),                 # ✅ 关键：numpy -> PIL
...
strong_transform = transforms.Compose([
    ...
    # ✅ RP/灰度图建议不要用 saturation（灰度图可能会报错或无意义）
...
def getDateset(dir_class1, dir_class2, dir_class3, testSize=0.3, rate=None, testNum=None, lessTran=False):
    ...
    if testNum == -1:
        ...
        dataset_weak = ImgDataset(X, Y, transform=weak_transform, lessTran=False)
        dataset_strong = ImgDataset(X, Y, transform=strong_transform, lessTran=False)
