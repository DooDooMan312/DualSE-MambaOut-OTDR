...
def get_args_parser():
    parser = argparse.ArgumentParser('Mamba training for image classification', add_help=False)

    # Model configuration
    parser.add_argument('--nb_classes', default=3, type=int, help='Number of classification types')
    parser.add_argument('--drop_path', default=0.1, type=float, metavar='PCT', help='Drop path rate (default: 0.1)')
    parser.add_argument('--global_pool', action='store_true')
    parser.set_defaults(global_pool=True)
    parser.add_argument('--model', default='vit_base_patch16', type=str, metavar='MODEL', help='Model to train')
...
# Update default configuration for the model
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 3, 'input_size': (1, 224, 224), 'pool_size': None,
        'crop_pct': 1.0, 'interpolation': 'bicubic',
        'mean': (33.9580), 'std': (68.0186), 'classifier': 'head',
        **kwargs
    }
...
def save_loss_acc_to_csv(epoch, train_loss, train_acc, val_loss, val_acc, elapsed_time, num_epoch):
    ...
    file_path_loss = f'model_save/weak_strong/{num_epoch}_loss_ws.csv'
    file_path_acc = f'model_save/weak_strong/{num_epoch}_acc_ws.csv'
...
def train_VAL(model, train_loader1, train_loader2, val_loader1, val_loader2, test_loader1, test_loader2, optimizer, loss_fn, batch_size
              , num_epoch, device,
              save_):
    ...
    for epoch in range(num_epoch):
        ...
        for (data1, target1), (data2, target2) in zip(train_loader1, train_loader2):
            ...
            input1, input2 = data1.to(device), data2.to(device)  # input：tensor(batch_size, channels, 224, 224)  ## input:(1, 224, 224)
    ...
    with torch.no_grad():
        ...
        print(classification_report(y_true_test, y_pred_test,digits=5, zero_division=0))
