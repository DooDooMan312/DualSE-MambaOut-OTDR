...
from train_v2_without_attention import train_VAL
...
def save_loss_acc_to_csv(epoch, train_loss, train_acc, val_loss, val_acc, elapsed_time, num_epoch):
    ...
    file_path_loss = f'model_save/{num_epoch}_withoutAttention_loss.csv'
    file_path_acc = f'model_save/{num_epoch}_withoutAttention_acc.csv'
...
def train_VAL(model, train_loader1, val_loader1, test_loader1, optimizer, loss_fn, batch_size, num_epoch, device,
    ...
    with torch.no_grad():
        ...
        # 打印测试集的最终指标
        print(f'Test Acc: {test_acc:.6f}, Loss: {test_loss:.6f}, test time: {test_time}')
