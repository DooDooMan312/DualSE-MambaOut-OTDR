...
#     if mode == 'train':
#         train_set = getDateset(class1path, class2path, class3path, testNum=-1)  # return Dataset-- 七种不同的transform，X，Y
#         # fixme
#         """
#         当 mode = 'train'时，会发生无法读取的问题
#         """
#         trainloader = DataLoader(train_set, batch_size=batchSize, shuffle=False)  # Dataset( batch_size =32 + train_set )
#
#         return trainloader
#
#
#     elif mode == 'test':
#         testset = getDateset(class1path, class2path, class3path, testNum=-1)
#         testLoader = DataLoader(testset, batch_size=1, shuffle=False)
#         return testLoader


# Adjust your getDataset function to handle LOOCV
def getDatasetLOOCV(class1path, class2path, class3path):
    x1, y1 = readjpgfile(class1path, 0)
    x2, y2 = readjpgfile(class2path, 1)
    x3, y3 = readjpgfile(class3path, 2)
...
# Implement LOOCV mechanism
def getDataLoaderLOOCV(class1path, class2path, class3path, batchSize, mode='train'):
    assert mode in ['train', 'val', 'test']

    X, Y = getDatasetLOOCV(class1path, class2path, class3path)
    ...
    for train_index, val_index in loo.split(X):
        ...
        val_dataset = ImgDataset(val_x, val_y, transform=test_transform)
